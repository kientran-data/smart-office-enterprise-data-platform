        ...
        try:
            with conn: # Context manager commits on success, rolls back on exception
                with conn.cursor() as cur:
                    ...
                    """, (doc_code, title, doc_type, creator['id'], datetime.now(), datetime.now()))
                    ...
                    for i, signer in enumerate(signers):
                        ...
                        """, (doc_id, creator['id'], signer['id'], i+1, simulation_time, datetime.now(), datetime.now()))
            ...
            with conn:
                with conn.cursor() as cur:
                    if req_status == 'SIGNED':
                        cur.execute("UPDATE signing.signing_requests SET status = 'SIGNED', signed_at = %s, updated_at = %s WHERE request_id = %s", (simulation_time, datetime.now(), req_id))
                        
                        cur.execute("SELECT COUNT(*) FROM signing.signing_requests WHERE document_id = %s AND status != 'SIGNED'", (doc_id,))
                        if cur.fetchone()[0] == 0:
                            cur.execute("UPDATE signing.documents SET status = 'COMPLETED', updated_at = %s WHERE document_id = %s", (datetime.now(), doc_id))
                            logger.info("SIGNING UPDATE document=%s COMPLETED", doc_id)
                    else:
                        cur.execute("UPDATE signing.signing_requests SET status = 'REJECTED', rejected_at = %s, updated_at = %s WHERE request_id = %s", (simulation_time, datetime.now(), req_id))
                        cur.execute("UPDATE signing.documents SET status = 'REJECTED', updated_at = %s WHERE document_id = %s", (datetime.now(), doc_id))
